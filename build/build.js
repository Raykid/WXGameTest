...
module.exports = function (data) {
    if (data && data.env) {
        if (data.env === "test" || data.env === "staging" || data.env === "release") {
            env = data.env;
        }
    }
    buildConfig.env = env;
    ...
    return new Promise((resolve, reject) => {
        ...
        // 处理game.js，并拷贝 index_wxgame.js 到 egret_wxgame 文件夹
        .then((data) => {
            return new Promise(resolve=>{
                console.log("-----------------------------------------------");
                console.log("拷贝game.js和index_wxgame.js到egret_wxgame文件夹");
                // 拷贝./game.js到egret_wxgame文件夹
                gulp.src(["./game.js", "./dist/index_wxgame.js"])
                    .pipe(gulp.dest("./egret_wxgame"))
                    .once("end", evt=>{
                        console.log("处理wxgame完毕");
                        resolve();
                    });
            });
        })
